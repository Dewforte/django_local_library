...
def index(request):
    ...
    num_fantasy_books = Book.objects.filter(genre=1).count()
    num_devotional_books = Book.objects.filter(genre=4).count()
    num_adventure_books = Book.objects.filter(genre=2).count()
    ...
    'num_devotional_books':num_devotional_books,
    'num_adventure_books':num_adventure_books,
...
class  AllLoanedBooks(LoginRequiredMixin, PermissionRequiredMixin, generic.ListView ):
    ...
    model = BookInstance
    template_name = 'catalog/all_loaned_bookinstance_list.html'
    paginate_by = 5
...
class BookCreate(CreateView, PermissionRequiredMixin, LoginRequiredMixin):
    permission_required = 'catalog.can_mark_returned'
    model = Book
    fields = '__all__'
